...
def save_barchart(total_scores, output_path, output_file_name, num_arms, separate_plots=False, group_seeds='mean', show=False, dist=7, yerr='std', psr_only=False):
    if separate_plots:
        ...
        for exp_idx, exp in enumerate(exps):
            ...
            if err is not 0:
                ax = plt.bar(0.5 + np.arange(len(labels)) + dist * exp_idx, vals, color=colors, capsize=8, yerr=np.abs(err))
            else:
        ...
        for exp, labels, vals in zip(exps, labels_out, vals_out):
            print(exp)
            for label, val in zip(labels, vals):
                if label == 'dr' or 'dmalt' in label:
                    print(label, val)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--group_seeds', type=str, choices=['mean', 'max', 'all'], default='mean', required=False, help='How to group seeds.')
    ...
    save_barchart(total_exp_scores, args.experiment_path, args.output_file_name,  num_arms=args.num_arms, group_seeds=args.group_seeds, show=args.show_plots, yerr=args.yerr, dist=args.spacing, psr_only=args.psr_only)
