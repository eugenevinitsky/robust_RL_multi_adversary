...
# Embedded file name: /Users/eugenevinitsky/Desktop/Research/Code/adversarial_sim2real/algorithms/baselines_her/multiagent_her/multiagent_her.py
# Compiled at: 2020-05-24 17:35:38
...
from algorithms.baselines_her.multiagent_her.rollout import RolloutWorker
...
def train(*, policy_dict, rollout_worker, evaluator, n_epochs, n_test_rollouts, n_cycles, n_batches, policy_save_interval, save_path, demo_file, **kwargs):
    ...
    for epoch in range(n_epochs):
        ...
        for i in range(n_cycles):
            ...
            if len(policy_dict) > 1:
                rollout_worker.venv.set_new_adversary(curr_adv)
            ...
            if len(policy_dict) > 1:
                temp_policy_dict = {'agent': policy_dict['agent'], 
                 'adversary{}'.format(rollout_worker.selected_adversary): policy_dict['adversary{}'.format(rollout_worker.selected_adversary)]}
            else:
        ...
        for policy_name, policy in policy_dict.items():
            for key, val in policy.logs():
                logger.record_tabular(policy_name + '_' + key, mpi_average(val))
...
def learn(*, network, env, total_timesteps, seed=None, eval_env=None, replay_strategy='future', policy_save_interval=5, clip_return=True, demo_file=None, override_params=None, load_path=None, save_path=None, num_adv=0, batch_size_scaling=1, **kwargs):
    ...
    for i in range(num_adv):
        policy_dict.update({'adversary{}'.format(i): config.configure_ddpg(dims=dims, params=(deepcopy(params_copy)), clip_return=clip_return,
...
@click.option('--batch_size_scaling', type=int, default=1, help='how much to scale up train batch size')
def main(**kwargs):
    learn(**kwargs)
...
if __name__ == '__main__':
    main()
# okay decompiling multiagent_her.cpython-36.pyc
