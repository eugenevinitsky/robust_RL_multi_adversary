...
class RockyLSTM(RecurrentTFModelV2):
    ...
    def __init__(self,
        ...
        # value fn
        v_state_in_h = tf.keras.layers.Input(shape=(self.cell_size, ), name="v_h")
        v_state_in_c = tf.keras.layers.Input(shape=(self.cell_size, ), name="v_c")

        v_lstm_out, v_state_h, v_state_c = tf.keras.layers.LSTM(
            self.cell_size, return_sequences=True, return_state=True, name="lstm")(
                inputs=input_layer,
                mask=tf.sequence_mask(seq_in),
                initial_state=[v_state_in_h, v_state_in_c])
    ...
    def forward_rnn(self, input_dict, state, seq_lens, prev_action=None):
        ...
        self._value_out, v_h, v_c = self.value_fn([input_dict, seq_lens] + state[2:4])
