...
from visualize.mujoco.transfer_tests import cheetah_grid, cheetah_mass_sweep, ant_run_list, hopper_run_list, hopper_friction_sweep, hopper_mass_sweep, ant_mass_sweep, ant_friction_sweep, cup_mass_sweep, ball_mass_sweep
...
if __name__ == '__main__':
    ...
    ###########################################################
    curr_path = os.path.abspath(__file__)
    data_dir = os.path.abspath(os.path.join(curr_path, '../../../data/cup')) + '/'
    ...
    for mass, fric in np.vstack((cup_grid[0].ravel(), cup_grid[1].ravel())).T:
        transfer_test_names.append('m_{}_f_{}'.format(mass, fric))


    output_files = 'final_plots/cup/cup_compare_valid_all_seeds.png'
    file_names = [data_dir + 'cup_0adv_concat1_grid/',
                       data_dir + 'cup_0adv_concat1_grid_dr/',
                       data_dir + 'cup_1adv_concat1_grid_str0p15/',
                       data_dir + 'cup_3adv_concat1_grid_str0p15_norew/',
                       data_dir + 'cup_5adv_concat1_grid_str0p15_norew/']
    legend_names = ['0 Adv', 'DR', '1 Adv', '3 Adv', '5 Adv']
    titles = ['Cup-Catch, Validation Set Reward']

    plot_across_seeds(file_names, transfer_test_names, output_files, legend_names, num_seeds=10,
                      yaxis=[0, 1000], titles=titles,
                      fontsize=fontsize, title_fontsize=title_fontsize, avg_across_tests=True, validation_set=True, use_std=True)
    output_files = ['final_plots/cup/cup_heat0adv.png', 'final_plots/cup/cup_heatdr.png', 'final_plots/cup/cup_heat1adv.png', 'final_plots/cup/cup_heat3adv.png', 'final_plots/cup/cup_heat5adv.png']

    plot_across_seeds_heatmap(exp_type='cup', mass_sweep=cup_mass_sweep, friction_sweep=ball_mass_sweep,  outer_folder_list=file_names, test_names=transfer_test_names, file_name=output_files,
                      titles=legend_names, num_seeds=10, title_fontsize=title_fontsize)
    #
    # # generate the test set maps for the best validation set result
    # test_names = [
