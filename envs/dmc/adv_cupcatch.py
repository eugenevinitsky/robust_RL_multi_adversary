...
class AdvMABallInCup(MultiAgentEnv):
    def __init__(self, config):
        ...
        self._env = dmc2gym.make(domain_name='ball_in_cup', task_name='catch')

        # used to track the previously observed states to induce a memory
        self.obs_size = 8 #4 position, 4 velocity
        self.num_actions = 2
        ...
        super(AdvMABallInCup, self).__init__()
        self.dr_idx_dict = {
            "cup_mass": 1,
            "ball_mass": 2,
        }
    ...
    def step(self, actions):
        ...
        if isinstance(actions, dict):
            ...
            obs_cup_action = actions['agent']
            cup_action = actions['agent']
        ...
        ob, reward, done, _ = self._env.step(cup_action)
        done = done or self.step_num >= self.horizon

        if self.concat_actions:
            self.update_observed_obs(np.concatenate((ob, obs_cup_action)))
        else:
