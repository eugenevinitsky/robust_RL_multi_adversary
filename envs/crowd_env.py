...
class MultiAgentCrowdSimEnv(CrowdSimEnv, MultiAgentEnv):
    ...
    def step(self, action, update=True):
        ...
        # check if reaching the goal
        end_position = np.array(self.robot.compute_position(action['av'], self.time_step))
        reaching_goal = norm(end_position - np.array(self.robot.get_goal_position())) < self.robot.radius
        ...
        if update:
            ...
            if self.robot.sensor == 'coordinates':
                ob = np.concatenate([human.get_next_observable_state(action['av']).as_array()
                                 for human, action['av'] in zip(self.humans, human_actions)]) / self.obs_norm
            elif self.robot.sensor == 'RGB':
                raise NotImplementedError

        av_ob = [adversary_action * o * 0.1 for o in ob] # 0.1 for dampening
        ob = {'av': av_ob, 'adversary': ob}
        reward = {'av': reward, 'adversary': -reward}
        done = {'__all__': done}
        # print('Step, ob: ', ob)
        # # print('Step, reward: ', reward)


        # ob = {'av': np.array([0, 0, 0]), 'adversary': np.array([0, 0, 0])}
        # reward = {'av': 0, 'adversary': 0}
        # done = {'__all__': done}
    ...
    def reset(self, phase='test', test_case=None):
        ...
        # print('Reset, ob is: ', ob)
        ob = {'av': ob, 'adversary': ob}
