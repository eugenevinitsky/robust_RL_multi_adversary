...
# Embedded file name: /Users/eugenevinitsky/Desktop/Research/Code/adversarial_sim2real/algorithms/baselines_her/multiagent_her/rollout.py
# Compiled at: 2020-05-24 20:38:32
...
class RolloutWorker:
    ...
    def generate_rollouts(self):
        ...
        obs, achieved_goals, acts, goals, successes = ([], [],
         {key:[] for key in self.policy.keys()}, [], [])
        dones = []
        ...
        if len(self.policy) > 1:
            temp_policy_dict = {'agent': self.policy['agent'], 
             'adversary{}'.format(self.selected_adversary): self.policy['adversary{}'.format(self.selected_adversary)]}
        else:
            temp_policy_dict = self.policy
        for t in range(self.T):
            for key, policy in temp_policy_dict.items():
                policy_output = policy.get_actions(o,
        ...
        for key in temp_policy_dict.keys():
            episode_dict.update({key: dict(o=obs, u=(acts[key]),
                    g=goals,
                    ag=achieved_goals)})
    ...
    def logs(self, prefix='worker'):
        ...
        return logs
# okay decompiling rollout.cpython-36.pyc
