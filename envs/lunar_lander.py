...
class AdvLunarLander(LunarLanderRandomized):
    ...
    def __init__(self, config):
        ...
        if len(config) > 0:
            ...
            self.should_render = config["render"]
            ...
            self.observation_space = spaces.Box(low=-np.inf, high=np.inf, shape=(self.observed_states.shape[0],))
    ...
    def adv_observation_space(self):
        return spaces.Box(-np.inf, np.inf, shape=(self.obs_size,), dtype=np.float32)
