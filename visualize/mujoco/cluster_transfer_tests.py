...
# now run sync_s3
os.chmod(os.path.expanduser("~/robust_RL_multi_adversary/run_scripts/s3_sync.sh"), 0o775)
p1 = subprocess.Popen(os.path.expanduser("~/robust_RL_multi_adversary/run_scripts/s3_sync.sh {}".format(exp_date)).split(' '))
p1.wait()
...
for (dirpath, dirnames, filenames) in os.walk(os.path.expanduser("~/s3_test")):
    if "checkpoint_{}".format(args.checkpoint_num) in dirpath and args.exp_title in dirpath:
        ...
        if config['env'] == "MALerrelPendulumEnv":
            ...
            lerrel_run_list = finger_run_list
