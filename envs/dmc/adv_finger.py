...
import dmc2gym
from visualize.plot_heatmap import spinner_mass_sweep, proximal_damping_sweep

from gym.utils import seeding

class AdvMAFinger(MultiAgentEnv):
    def __init__(self, config):
        ...
        self._env = dmc2gym.make(domain_name='finger', task_name='turn_hard')

        # used to track the previously observed states to induce a memory
        self.obs_size = 12 #4 position, 4 velocity
        self.num_actions = 2
        ...
        super(AdvMAFinger, self).__init__()
        self.dr_idx_dict = {
            "spinner_mass": 3,
            "proximal_damping": 0
        }
        self.original_params = {
            "spinner_mass": 2.32478,  # Significant changes
    ...
    def randomize_domain(self):
        for param, idx in self.dr_idx_dict.items():
            if 'spinner' in param:
                coeff = np.random.choice(spinner_mass_sweep)
                self._env.physics.model.body_mass[idx] = self.original_params[param] * coeff

            elif 'proximal' in param:
                coeff = np.random.choice(proximal_damping_sweep)
                self._env.physics.model.dof_damping[idx] = self.original_params[param] * coeff
    ...
    def step(self, actions):
        ...
        if isinstance(actions, dict):
            ...
            obs_finger_action = actions['agent']
            finger_action = actions['agent']
            ...
            if self.adversary_range > 0 and 'adversary{}'.format(self.curr_adversary) in actions.keys():
                ...
                finger_action += adv_action
                # apply clipping to ant action
                if self.clip_actions:
                    finger_action = np.clip(finger_action, a_min=self.action_space.low,
                                            a_max=self.action_space.high)
        else:
            assert actions in self.action_space
            obs_finger_action = actions
        ...
        ob, reward, done, _ = self._env.step(finger_action)
        done = done or self.step_num >= self.horizon

        if self.concat_actions:
            self.update_observed_obs(np.concatenate((ob, obs_finger_action)))
        else:
...
def finger_env_creator(env_config):
    env = AdvMAFinger(env_config)
    return env